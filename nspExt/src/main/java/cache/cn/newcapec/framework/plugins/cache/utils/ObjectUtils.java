...
public class ObjectUtils {
	...
	public static void convertStringFieldCoding(Object theObject) {
		...
		for (int i = 0, n = propDescs.length; i < n; i++) {
			...
			// log.debug("1111 name ==" + name +", value == " +
			...
			if (value instanceof String) {
				value = StringUtils.convertStrByCoding((String) value,
						"ISO-8859-1");
				...
				for (int index = 0; index < strs.length; index++) {
					...
					values[index] = StringUtils.convertStrByCoding(
							(String) strs[index], "ISO-8859-1");
				}
				value = values;
			} else {
				continue;
			...
			// log.debug("2222 name ==" + name +", value == " +
			// ObjectUtils.nullSafeToString(value));
	...
	/**
	 * @see #toString() �?个�?�用的toString方法 将一个POJO类的 getXXX 的�?�全部输�?
	 * */
	public static String toStringMethod(Object theObject) {
		try {
			...
			log.error(theObject.getClass().getCanonicalName()
					+ " toString() Error!", e);
			return "";
		}
	}
	...
	public static int nullSafeHashCode(Object obj) {
		return org.springframework.util.ObjectUtils
	...
	public static String getFieldName(Class<?> clazz, Class<?> fieldType) {
		...
		throw new IllegalArgumentException("The Field ["
				+ fieldType.getCanonicalName() + "] is not in Object ["
