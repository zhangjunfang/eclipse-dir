...
public abstract class ArrayUtils {
	...
	public static <T> T[] copyArray(T[] src) {
		...
		T[] result = (T[]) Array.newInstance(src.getClass().getComponentType(),
				length);
	 ...
	 * */
	@SuppressWarnings("unchecked")
	public static <T> T[] addObjectsToArray(T[] array, int index,
			T... otherObjs) {
		if (otherObjs == null || array == null) {
			throw new NullPointerException();
		}
		...
		if (index < 0 || index > array.length) {
			throw new IndexOutOfBoundsException("索引:[" + index
					+ "]小于0 或�?? 大于数组长度: [" + array.length + "]");
		}

		T[] result = (T[]) Array.newInstance(array.getClass()
				.getComponentType(), array.length + otherObjs.length);

		System.arraycopy(array, 0, result, 0, index);
		System.arraycopy(otherObjs, 0, result, index, otherObjs.length);
		System.arraycopy(array, index, result, index + otherObjs.length,
				array.length - index);
	...
	/**
	 * 合并对象到一个对象数组Object[] �? 如果对象�? null, String, Number, Date 类型则直接加�? 如果对象�?
	 * getClass().isArray() 则将每项加入
	 * */
	public static Object[] mergeObjectsToArray(Object... objs) {
		...
		for (Object obj : objs) {
			if (ArrayUtils.addIntoList(list, obj)) {
				continue;
			}
			...
			if (obj.getClass().isArray()) {
				...
				for (int i = 0; i < Array.getLength(obj); i++) {
					...
					if (ArrayUtils.addIntoList(list, temp)) {
						continue;
					}
					throw new UnsupportedOperationException(
							"mergeObjectsToArray 不支持类型["
	...
	/**
	 * 避免�? ArrayIndexOutOfBoundsException �? NullPointerException
	 * <p>
	 * safeGet(null, 0) = null
	 * <p>
	 * safeGet(int[]{1,2,3}, -1) = null
	 * <p>
	 * safeGet(int[]{1,2,3}, 3) = null
	 * <p>
	 * safeGet(int[]{1,2,3}, 2) = 3
	 * <p>
	 * */
	...
	/**
	 * 返回�?个新的数组实例并且填充默认�?? newInstanceWithFill(String.class, 2, "s") = new
	 * String[]{"s", "s"}
	 * <p>
	 * newInstanceWithFill(Integer.class, 3, 1) = new Integer[]{new Integer(1),
	 * new Integer(1), new Integer(1)}
	 * <p>
	 * newInstanceWithFill(Integer.TYPE, 3, 1) = new int[]{1, 1, 1}
	 * <p>
	 * */
	public static Object newInstanceWithFill(Class<?> componentType,
			int length, Object filledValue) {
	...
	public static void main(String[] args) {
		...
		// log.debug(ObjectUtils.nullSafeToString(newInstanceWithFill(Integer.TYPE,
		// 2 , 3)));
		// log.debug(newInstanceWithFill(Integer.class, 2 , 3) instanceof
		// int[]);
		// log.debug(ObjectUtils.nullSafeToString(newInstanceWithFill(Integer.class,
		// 2 , 3)));
	}
}
