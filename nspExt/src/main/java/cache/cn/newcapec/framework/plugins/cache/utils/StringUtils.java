...
public abstract class StringUtils {
	...
	public static void bulkUpdate(String[] array, String leading,
			String trailing) {
	 ...
	 *            尾标�?
	 * */
	public static <T> String arrayToString(T[] array, String leading,
			String trailing) {
		return arrayToString(array, ",", leading, trailing);
	}

	/**
	 * 数组转化成字符串, 使用自定义的连接符号 并加上自定义的首 尾标�?
	 * 
	 * @param connectSymbol
	 *            连接符号
	 * @param leading
	...
	public static <T> String arrayToString(T[] array, String connectSymbol,
		...
		if (len == 0)
			return "";
		StringBuffer buf = new StringBuffer(len * 12 + leading.length()
				+ trailing.length());
		for (int i = 0; i < len - 1; i++) {
			buf.append(leading).append(array[i].toString()).append(trailing)
					.append(connectSymbol);
		}
		return buf.append(leading).append(array[len - 1].toString())
				.append(trailing).toString();
	}
	...
	/**
	 * 将str由code1转换为code2 如果str为空，则返回"";
	 * **/
	public static String converStrByCoding(String str, String code1,
			String code2) {
	...
	public static String collectionToString(Collection<?> coll,
			String connectSymbol, String leading, String trailing) {
	...
	public static List<String> collectionToStringList(Collection<?> coll) {
		...
		List<String> result = new ArrayList<String>(coll.size() > 10000 ? 10000
				: coll.size());
	...
	/**
	 * 在数组的指定位置插入指定个数的元�?, <br>
	 * 将当前处于该位置的元素（如果有的话）和所有后续元素向右移动（在其索引中加指定个数).
	 * 
	 * @param array
	 *            源数�?
	 * @param index
	...
	public static String[] addToArray(String[] array, int index, String... strs) {
		...
		System.arraycopy(array, index, result, index + strs.length,
				array.length - index);
	...
	public static String outByTimes(String s, int times) {
		...
		StringBuilder builder = new StringBuilder(
				s.length() * times > 2000 ? 2000 : s.length() * times);
	...
	/**
	 * 获得汉字拼音的首字母,如果参数不符号要�? 则返回自�?(单字�?) or ' '(无法解析的双字符)
	 * */
	public static char getPYHeadLetter(char aChar) {
		char result;
		// boolean flag = true;
		int charAscii = getCharAscii(aChar);
		if (0 <= charAscii && charAscii <= 255)
			result = aChar; // Ascii 小于 256 返回自身
