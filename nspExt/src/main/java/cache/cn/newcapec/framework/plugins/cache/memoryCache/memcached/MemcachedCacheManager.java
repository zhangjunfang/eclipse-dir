...
public class MemcachedCacheManager implements ICacheManager<IMemcachedCache> {
	private static final Log Logger = LogFactory
			.getLog(MemcachedCacheManager.class);
	...
	public void start() {
		...
		if (memcachedClientconfigs != null && memcachedClientconfigs.size() > 0
				&& memcachedClientSocketPoolConfigs != null
			...
			if (supportMultiConfig) {
				...
				if (urls == null || !urls.hasMoreElements()) {
					...
					throw new java.lang.RuntimeException(
							"no memcached config find! please put memcached.xml in your classpath");
				}
				...
				while (urls.hasMoreElements()) {
					...
					CacheUtil.loadMemcachedConfigFromURL(url, factory,
							...
							memcachedClientClusterConfigs);

					Logger.info(new StringBuilder()
							.append("load config from :").append(url.getFile()));
				}
			} else {
				URL url = null;
				ClassLoader loader = Thread.currentThread()
				...
				Logger.info(new StringBuilder().append("load config from :")
						.append(url.getFile()));
			}
	...
	protected void initMemCacheClientPool() {
		...
		for (MemcachedClientSocketPoolConfig socketPool : memcachedClientSocketPoolConfigs) {
			if (socketPool != null && socketPool.getServers() != null
				...
				SockIOPool pool = SockIOPool.getNewInstance(socketPool
						.getName());
				...
				if (socketpool.get(socketPool.getName()) != null)
					Logger.error(new StringBuilder(
							"socketpool define duplicate! socketpool name:")
							.append(socketPool.getName()));

				socketpool.put(socketPool.getName(), pool);
				Logger.info(new StringBuilder().append(" add socketpool :")
						.append(socketPool.getName()));
		...
		for (MemcachedClientConfig node : memcachedClientconfigs) {
			...
			IMemcachedCache cache = new MemcachedCache(helper,
			...
			try {
				if (node.getErrorHandler() != null
					...
					client.setErrorHandler((ErrorHandler) Class.forName(
			...
			if (cachepool.get(node.getName()) != null)
				Logger.error(new StringBuilder(
						"cache define duplicate! cache name :").append(node
						.getName()));

			cachepool.put(node.getName(), cache);
			Logger.info(new StringBuilder().append(" add memcachedClient :")
					.append(node.getName()));
		}
		...
		for (MemcachedClientClusterConfig node : memcachedClientClusterConfigs) {
			...
			if (clients != null && clients.length > 0) {
				...
				if (clusterpool.get(cluster.getName()) != null)
					Logger.error(new StringBuilder(
							"cluster define duplicate! cluster name :")
