...
public class CacheUtil {
	...
	public static <B extends ICache<?, ?>> ICacheManager<B> getCacheManager(
			Class<B> cache, String defaultImplClass) {
		ICacheManager<B> cacheManager = null;

		cacheManager = (ICacheManager<B>) getInstanceByInterface(
				ICacheManager.class, Thread.currentThread()
						.getContextClassLoader(), defaultImplClass, true);
	...
	public static <I> I getInstanceByInterface(Class<I> interfaceDefinition,
		...
		// 获取缓存的情况，或者移除缓存
		if (needCache) {
			String className = (String) localCache.get(interfaceDefinition
					.getName());
		...
		try {
			String systemProp = System.getProperty(interfaceDefinition
					.getName());
			if (systemProp != null) {
		...
		String serviceId = new StringBuilder("META-INF/services/").append(
				interfaceDefinition.getName()).toString();
		...
		try {
			...
			if (in != null) {
				...
				if (className != null && !"".equals(className)) {
					...
					if (needCache)
						localCache
								.put(interfaceDefinition.getName(), className);

					return newInstance(interfaceDefinition, className,
							classLoader);
				}
			} else {
				if (defaultImplClass != null && !defaultImplClass.equals("")) {
					...
					if (needCache)
						localCache.put(interfaceDefinition.getName(),
								defaultImplClass);

					return newInstance(interfaceDefinition, defaultImplClass,
							classLoader);
				}
			}
	 ...
	 */
	public static void loadMemcachedConfigFromURL(
			URL url,
		...
		try {
				...
				if (event.isStartElement()) {
					...
					if (tag.equalsIgnoreCase("client")) {
						...
						if (start.getAttributeByName(new QName("", "name")) != null)
							node.setName(start.getAttributeByName(
									new QName("", "name")).getValue());
						else
							throw new RuntimeException(
									"memcached client name can't not be null!");

						if (start
								.getAttributeByName(new QName("", "socketpool")) != null)
							node.setSocketPool(start.getAttributeByName(
									new QName("", "socketpool")).getValue());
						else
							throw new RuntimeException(
									"memcached client socketpool can't not be null!");

						if (start.getAttributeByName(new QName("",
								"compressEnable")) != null)
							node.setCompressEnable(Boolean.parseBoolean(start
									.getAttributeByName(
											new QName("", "compressEnable"))
									.getValue()));
						else
						...
						if (start.getAttributeByName(new QName("",
							...
							node.setDefaultEncoding(start.getAttributeByName(
									new QName("", "defaultEncoding"))
									.getValue());
						else
					...
					if (tag.equalsIgnoreCase("socketpool")) {
						...
						if (start.getAttributeByName(new QName("", "name")) != null)
							socketnode.setName(start.getAttributeByName(
									new QName("", "name")).getValue());
						else
							throw new RuntimeException(
									"memcached client socketpool name can't not be null!");

						if (start.getAttributeByName(new QName("", "failover")) != null)
							socketnode.setFailover(Boolean.parseBoolean(start
									.getAttributeByName(
											new QName("", "failover"))
									.getValue()));

						if (start.getAttributeByName(new QName("", "initConn")) != null)
							socketnode.setInitConn(Integer.parseInt(start
									.getAttributeByName(
											new QName("", "initConn"))
									.getValue()));

						if (start.getAttributeByName(new QName("", "minConn")) != null)
							socketnode.setMinConn(Integer.parseInt(start
									.getAttributeByName(
											new QName("", "minConn"))
									.getValue()));

						if (start.getAttributeByName(new QName("", "maxConn")) != null)
							socketnode.setMaxConn(Integer.parseInt(start
									.getAttributeByName(
											new QName("", "maxConn"))
									.getValue()));

						if (start
								.getAttributeByName(new QName("", "maintSleep")) != null)
							socketnode.setMaintSleep(Integer.parseInt(start
									.getAttributeByName(
											new QName("", "maintSleep"))
									.getValue()));

						if (start.getAttributeByName(new QName("", "nagle")) != null)
							socketnode.setNagle(Boolean.parseBoolean(start
									.getAttributeByName(new QName("", "nagle"))
									.getValue()));

						if (start.getAttributeByName(new QName("", "socketTO")) != null)
							socketnode.setSocketTo(Integer.parseInt(start
									.getAttributeByName(
											new QName("", "socketTO"))
									.getValue()));

						if (start.getAttributeByName(new QName("", "maxIdle")) != null)
							socketnode.setMaxIdle(Integer.parseInt(start
									.getAttributeByName(
											new QName("", "maxIdle"))
									.getValue()));

						if (start
								.getAttributeByName(new QName("", "aliveCheck")) != null)
							socketnode.setAliveCheck(Boolean.parseBoolean(start
									.getAttributeByName(
											new QName("", "aliveCheck"))
									.getValue()));
					...
					if (tag.equalsIgnoreCase("cluster")) {
						...
						if (start.getAttributeByName(new QName("", "name")) != null)
							clusternode.setName(start.getAttributeByName(
									new QName("", "name")).getValue());
						else
							throw new RuntimeException(
									"memcached cluster name can't not be null!");

						if (start.getAttributeByName(new QName("", "mode")) != null)
							clusternode.setMode(start.getAttributeByName(
									new QName("", "mode")).getValue());

						continue;
					}

					if (tag.equalsIgnoreCase("memCachedClients")
							&& clusternode != null) {
						event = r.peek();
						...
						if (event.isCharacters()) {
							...
							if (clients != null && !clients.equals("")) {
								clusternode.setMemCachedClients(clients
										.split(","));
							}
							r.nextEvent();
						}
				...
				if (event.isEndElement()) {
					...
					if (node != null
						...
						Logger.info(new StringBuilder().append(
								" add memcachedClient config :").append(
								node.getName()));
					...
					if (socketnode != null
							&& end.getName().getLocalPart()
									.equalsIgnoreCase("socketpool")) {
						memcachedClientSocketPoolConfigs.add(socketnode);
						Logger.info(new StringBuilder().append(
								" add socketpool config :").append(
								socketnode.getName()));
			...
			Logger.error(new StringBuilder(
					"MemcachedManager loadConfig error !")
