...
public class TriggerResource extends BaseResource {
	...
	public void triggerListUI(BaseRequest request, BaseResponse response) {
		...
		response.toView(getUrl("trigger.triggerListUI"),
				getNewcapectViewContext());
	}

	/**
	 * @author 景明超 2014-1-2 下午1:37:51
	 * @param request
	 * @param response
	 */
	public void triggerListGrid(BaseRequest request, BaseResponse response) {
		...
		if (triggers != null && !triggers.isEmpty()) {
			for (Iterator<String> iter = triggers.keySet().iterator(); iter
					.hasNext();) {
		...
		PageView<TriggerForm> pageView = new PageView<TriggerForm>(
		...
		response.toView(getUrl("trigger.triggerListGrid"),
				getNewcapectViewContext());
	}
	...
	private Map<String, List<TriggerForm>> getTriggers() throws Exception {
		...
		if (triggerGroupNames != null) {
			for (String groupName : triggerGroupNames) {
				...
				Set<TriggerKey> triggerKeys = this.scheduler
						.getTriggerKeys(GroupMatcher
								.triggerGroupEquals(groupName));
				if (triggerKeys != null) {
					for (TriggerKey triggerKey : triggerKeys) {
						...
						form.setNextFireTime(DateUtil.toDatetimeString(trigger
								.getNextFireTime()));
						form.setStartTime(DateUtil.toDatetimeString(trigger
								.getStartTime()));
						form.setStopTime(DateUtil.toDatetimeString(trigger
								.getEndTime()));
						form.setPreviousFireTime(DateUtil
								.toDatetimeString(trigger.getPreviousFireTime()));
						form.setMisFireInstruction(trigger
