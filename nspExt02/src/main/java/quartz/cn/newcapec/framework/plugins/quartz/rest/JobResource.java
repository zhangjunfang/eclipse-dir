...
public class JobResource extends BaseResource {
	...
	public void jobListGrid(BaseRequest request, BaseResponse response) {
		...
		if (StringUtils.isNotBlank(jobName) && jobDetails != null
				&& !jobDetails.isEmpty()) {
			for (Iterator<String> iter = jobDetails.keySet().iterator(); iter
					.hasNext();) {
		...
		PageView<JobDetailForm> pageView = new PageView<JobDetailForm>(
				PageContext.getPagesize(), PageContext.getOffset());
		if (PageContext.getOffset() == 0) {
			PageContext.setOffset(1);
		}
		int fromIndex = (PageContext.getOffset() - 1)
	...
	public void view(BaseRequest request, BaseResponse response) {
		...
		for (Trigger trigger : triggers) {
			...
			tForm.setStartTime(DateUtil.toDateTimeString(
					trigger.getStartTime(), DateUtil.DATETIME_FORMAT));
			tForm.setStopTime(DateUtil.toDateTimeString(trigger.getEndTime(),
					DateUtil.DATETIME_FORMAT));
			tForm.setTriggerGroup(trigger.getKey().getGroup());
			tForm.setTriggerName(trigger.getKey().getName());
			tForm.setNextFireTime(DateUtil.toDateTimeString(
					trigger.getNextFireTime(), DateUtil.DATETIME_FORMAT));
			tForm.setPreviousFireTime(DateUtil.toDateTimeString(
					trigger.getPreviousFireTime(), DateUtil.DATETIME_FORMAT));
		...
		for (JobListener jobListener : jls) {
			...
			List<Matcher<JobKey>> matchers = listenerManager
					.getJobListenerMatchers(jobListener.getName());
	...
	public void editJobUI(BaseRequest request, BaseResponse response) {
		...
		if (name.length() > 0 && group.length() > 0) {
			...
			try {
				...
				jobBean.setDisallowConcurrent(jobDetail
						.isConcurrentExectionDisallowed());
				jobBean.setPersistJobData(jobDetail
						.isPersistJobDataAfterExecution());

				Map<String, String> parameters = new java.util.LinkedHashMap<String, String>();
				for (Map.Entry<String, Object> entry : jobDetail
						.getJobDataMap().entrySet()) {
	...
	public void removeJobTrigger(BaseRequest request, BaseResponse response) {
		response.print(doExpAssert(new AssertObject() {
			...
			public void AssertMethod(Msg msg) {
				String triggerName = JSONTools.getString(getJsonObject(),
						"triggerName");
				String triggerGroup = JSONTools.getString(getJsonObject(),
						"triggerGroup");
				try {
	...
	public void removeJobListener(BaseRequest request, BaseResponse response) {
		response.print(doExpAssert(new AssertObject() {
			@Override
			public void AssertMethod(Msg msg) {
				String listenerName = JSONTools.getString(getJsonObject(),
						"listenerName");
				String name = JSONTools.getString(getJsonObject(), "name");
				try {
					Matcher<JobKey> jobkeyMatcher = NameMatcher
							.nameEndsWith(name);
					scheduler.getListenerManager().removeJobListenerMatcher(
							listenerName, jobkeyMatcher);
					msg.setSuccess(true);
				} catch (SchedulerException e) {
					log.error(null, e);
					msg.setSuccess(false);
	...
	public void add(BaseRequest request, BaseResponse response) {
		response.print(doExpAssert(new AssertObject() {
			...
			public void AssertMethod(Msg msg) {
				String name = JSONTools.getString(getJsonObject(), "name");
				String group = JSONTools.getString(getJsonObject(), "group");
				String jobClass = JSONTools.getString(getJsonObject(),
						"jobClass");
				String description = JSONTools.getString(getJsonObject(),
						"description");
				boolean recovery = JSONTools.getBoolean(getJsonObject(),
						"recovery");
				boolean durable = JSONTools.getBoolean(getJsonObject(),
						"durable");
				JSONObject parameters = JSONObject.fromObject(JSONTools
						.getString(getJsonObject(), "parameters"));
	...
	public void delete(BaseRequest request, BaseResponse response) {
		response.print(doExpAssert(new AssertObject() {
			@Override
			public void AssertMethod(Msg msg) {
				JSONArray jsonArray = JSONArray.fromObject(JSONTools.getString(
						getJsonObject(), "ids"));
	...
	public void executeSimpleUI(BaseRequest request, BaseResponse response) {
		...
		response.toView(getUrl("job.executeSimpleUI"),
				getNewcapectViewContext());
	}
	...
	public void executeSimple(BaseRequest request, BaseResponse response) {
		response.print(doExpAssert(new AssertObject() {
			...
			public void AssertMethod(Msg msg) {
				...
				String description = JSONTools.getString(getJsonObject(),
						"description");
				String startTime = JSONTools.getString(getJsonObject(),
						"startTime");
				String endTime = JSONTools
						.getString(getJsonObject(), "endTime");
				int repeatCount = JSONTools.getInt(getJsonObject(),
						"repeatCount");
				int repeatInterval = JSONTools.getInt(getJsonObject(),
						"repeatInterval");
				...
				if (StringUtils.isNotBlank(startTime)) {
					try {
						simpleTrigger.setStartTime(DateUtils.parseDate(
								startTime, new String[] { "yyyy-MM-dd HH:mm" }));
				...
				if (StringUtils.isNotBlank(endTime)) {
					try {
						simpleTrigger.setEndTime(DateUtils.parseDate(endTime,
	...
	public void executeCron(BaseRequest request, BaseResponse response) {
		response.print(doExpAssert(new AssertObject() {
			...
			public void AssertMethod(Msg msg) {
				...
				if (StringUtils.isNotBlank(startTime)) {
					try {
						cronTrigger.setStartTime(DateUtils.parseDate(startTime,
								new String[] { "yyyy-MM-dd HH:mm" }));
					} catch (ParseException e) {
						log.error(null, e);
					}
				...
				if (StringUtils.isNotBlank(endTime)) {
					try {
						cronTrigger.setEndTime(DateUtils.parseDate(endTime,
								new String[] { "yyyy-MM-dd HH:mm" }));
					} catch (ParseException e) {
						log.error(null, e);
					}
	...
	public void testCronExpression(BaseRequest request, BaseResponse response) {
		response.print(doExpAssert(new AssertObject() {
			@Override
			public void AssertMethod(Msg msg) {
				String cronExp = JSONTools
						.getString(getJsonObject(), "cronExp");
				...
				String beginDate = DateFormatUtils.format(curentTime,
						"yyyy-MM-dd HH:mm:ss");
				...
				for (int i = 1; i <= 8; i++) {
					...
					fireTime.add(DateFormatUtils.format(validTime,
							"yyyy-MM-dd HH:mm:ss"));
				}
