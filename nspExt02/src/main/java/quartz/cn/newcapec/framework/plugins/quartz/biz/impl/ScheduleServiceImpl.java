...
public class ScheduleServiceImpl implements ScheduleService, InitializingBean {
	...
	@Override
	public void schedule(JobDetail jobDetail, String name,
			CronExpression cronExpression, String group) {
	...
	public void schedule(JobDetail jobDetail, Date startTime, Date endTime,
			int repeatCount) {
		schedule(jobDetail, startTime, endTime, repeatCount,
	...
	public void schedule(JobDetail jobDetail, String name, Date startTime,
		...
		schedule(jobDetail, name, startTime, endTime, repeatCount,
				Scheduler.DEFAULT_GROUP);
	}

	@Override
	public void schedule(JobDetail jobDetail, String name, Date startTime,
			Date endTime, int repeatCount, String group) {
	...
	public void schedule(JobDetail jobDetail, Date startTime, Date endTime,
		...
		schedule(jobDetail, startTime, endTime, repeatCount, repeatInterval,
				Scheduler.DEFAULT_GROUP);
	}
	...
	public void schedule(JobDetail jobDetail, Date startTime, Date endTime,
		...
		schedule(jobDetail, null, startTime, endTime, repeatCount,
				repeatInterval, group);
	}

	@Override
	public void schedule(JobDetail jobDetail, String name, Date startTime,
			Date endTime, int repeatCount, long repeatInterval) {
		this.schedule(jobDetail, name, startTime, endTime, repeatCount,
				repeatInterval, Scheduler.DEFAULT_GROUP);
	}

	@Override
	public void schedule(JobDetail jobDetail, String name, Date startTime,
			Date endTime, int repeatCount, long repeatInterval, String group) {
		if (name == null || name.trim().equals("")) {
			name = UUID.randomUUID().toString();
		}
		try {
			...
			quartzScheduler
					.rescheduleJob(simpleTrigger.getKey(), simpleTrigger);
		} catch (SchedulerException e) {
			throw new RuntimeException(e);
		}
	}
	...
	public void pauseTrigger(String triggerName, String triggerGroup) {
		try {
			quartzScheduler.pauseTrigger(new TriggerKey(triggerName,
					triggerGroup));// 停止触发器
		} catch (SchedulerException e) {
			throw new RuntimeException(e);
		}
	...
	public void resumeTrigger(String triggerName, String triggerGroup) {
		try {
			quartzScheduler.resumeTrigger(new TriggerKey(triggerName,
					triggerGroup));// 重启触发器
		} catch (SchedulerException e) {
			throw new RuntimeException(e);
		}
	...
	public boolean removeTrigger(String triggerName, String triggerGroup) {
		try {
			quartzScheduler.pauseTrigger(new TriggerKey(triggerName,
					triggerGroup));// 停止触发器
			return quartzScheduler.unscheduleJob(new TriggerKey(triggerName,
					triggerGroup));// 移除触发器
		} catch (SchedulerException e) {
			throw new RuntimeException(e);
		}
	...
	public void afterPropertiesSet() throws Exception {
		org.springframework.util.Assert.notNull(this.quartzScheduler,
				"properties 'quartzScheduler' is requirt.");
	}
	...
	public boolean jobExisting(String jobName, String jobGroup) {
		try {
			return this.quartzScheduler.checkExists(new JobKey(jobName,
					jobGroup));
		} catch (SchedulerException e) {
			throw new RuntimeException(e);
		}
	...
	public boolean triggerExisting(String triggerName, String triggerGroup) {
		try {
			return this.quartzScheduler.checkExists(new TriggerKey(triggerName,
					triggerGroup));
		} catch (SchedulerException e) {
			throw new RuntimeException(e);
		}
