...
public class SchedulerResource extends BaseResource {
	...
	public void schedulerUI(BaseRequest request, BaseResponse response) {
		...
		try {
			...
			if (command.equals("start")) {
				...
				scheduleInfo.setMessage(StringUtils.isBlank(command) ? "无"
						: "命令‘" + command + "’不存在。");
			}
		} catch (SchedulerException e) {
			scheduleInfo.setMessage("命令‘" + command + "’执行失败，失败原因："
					+ e.getMessage());
		...
		response.toView(getUrl("scheduler.schedulerUI"),
				getNewcapectViewContext());
	}

	/**
	 * @author 景明超 2014-1-2 下午3:22:57
	 * @param scheduler2
	 * @param scheduleInfo
	...
	private void populateSchedulerForm(Scheduler scheduler,
		...
		schedForm.setNumJobsExecuted(String.valueOf(scheduler.getMetaData()
		...
		schedForm.setRunningSince(DateUtil.toDatetimeString(scheduler
				.getMetaData().getRunningSince()));
		...
		schedForm.setThreadPoolSize(String.valueOf(scheduler.getMetaData()
				.getThreadPoolSize()));
		schedForm.setVersion(scheduler.getMetaData().getVersion());
		schedForm.setSummary(scheduler.getMetaData().getSummary());

		List<JobExecutionContext> jobDetails = scheduler
				.getCurrentlyExecutingJobs();
		form.setExecutingJobs(new ArrayList<JobDetailForm>());
		for (Iterator<JobExecutionContext> iter = jobDetails.iterator(); iter
				.hasNext();) {
		...
		List<JobListener> jobListeners = scheduler.getListenerManager()
				.getJobListeners();
		for (Iterator<JobListener> iter = jobListeners.iterator(); iter
				.hasNext();) {
