...
public class NspLoggerImpl implements NspLogger {
	...
	public void error(OperateLog log) {
		if (this.logger.isErrorEnabled()) {
			this.logger.error(MSG_FORMATE, new Object[] { log.getUsername(),
					log.getTitle(), log.getDescription() });
	...
	public void warn(OperateLog log) {
		if (this.isWarnEnabled()) {
			this.logger.warn(
					MSG_FORMATE,
					new Object[] { log.getUsername(), log.getTitle(),
							log.getDescription() });
			OperateLogUtils.getOperateLogService().pushLog(log);
		}
	}
	...
	private OperateLog createSystemLog(Operator operator, String title,
			String desc) {
