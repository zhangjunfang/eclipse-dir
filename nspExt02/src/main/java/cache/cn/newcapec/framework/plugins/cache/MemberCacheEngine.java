...
public class MemberCacheEngine extends AbstractCacheEngine {
	...
	public void init() {
		try {
			...
			pool.setServers(this.getServers().toArray(
					new String[this.getServers().size()]));
			pool.setWeights(this.getLevels().toArray(
					new Integer[this.getLevels().size()]));
	...
	public void loadJndi() {
		...
		try {
			...
			cs.setDefaultTime((Long) PropertyUtils
					.getProperty(o, "defaultTime"));
			cs.setSocketTo((Integer) PropertyUtils.getProperty(o, "socketTo"));
			cs.setSocketConnectTO((Integer) PropertyUtils.getProperty(o,
					"socketConnectTO"));
			cs.init();
			log.info("加载缓存配置器成功!");
