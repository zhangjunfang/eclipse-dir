...
public abstract class NumberUtils {
	...
	public static String format(Object obj, String pattern) {
		if (obj == null || !NumberUtils.canParseDouble(obj)) {
			return "";
		}
	...
	/**
	 * 将int转成指定位数(如果需要转)的String 如果int本身位数大于所需, 则返回int
	 * */
	...
	public static Integer toInteger(Object obj, boolean isThrowException) {
		...
		try {
			...
			if (isThrowException) {
				throw new IllegalArgumentException("["
						+ ObjectUtils.nullSafeToString(obj) + "]转换Integer错误!",
						e);
			} else {
				i = null;
			}
	...
	public static Long toLong(Object obj, boolean isThrowException) {
		...
		try {
			...
			if (isThrowException)
				throw new IllegalArgumentException("["
						+ ObjectUtils.nullSafeToString(obj) + "]转换Long错误!", e);
		}
		return l;
	}
	...
	public static Short toShort(Object obj, boolean isThrowException) {
		...
		try {
			...
			if (isThrowException)
				throw new IllegalArgumentException("["
						+ ObjectUtils.nullSafeToString(obj) + "]转换Short错误!", e);
		}
		return s;
	}
	...
	public static int intValue(Object obj, int defaultValue) {
		return toInteger(obj, false) == null ? defaultValue : toInteger(obj,
				false).intValue();
	}
	...
	public static long longValue(String str, long defaultValue) {
		return toLong(str, false) == null ? defaultValue : toLong(str)
				.longValue();
	}
