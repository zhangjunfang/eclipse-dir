...
/* 打印及时消息的内容 */
function onMsgPrint() {
	var docStr = document.getElementById("msgDivZ").innerHTML;
	docStr += document.getElementById("msgDiv0").innerHTML;
	...
	jQuery("#form0").ajaxSubmit(
			{
				...
				success : function(result) {
					if (result.success) {
						...
						jQuery("#currOddFare").html(auth);
						jQuery("#costMoney").val(auth);
						jQuery("#currCntIn").html(dt.msgForm.currCntIn);
						jQuery("#currCntOut").html(dt.msgForm.currCntOut);
