...
function reloadList(offset){
	...
	var params = {offset:offset,netsiteid:jQuery("#netsiteid").val(),template_type:jQuery("#templateType").val(),template_name:jQuery("#templateName").val().trim()};// 拼json参数
	jQuery("#pageList").load("listTemplateData", params,null);
}
//提交复制模板
function submitTemplatePara(){
	if (isRepeat) {
		alert('该名称的打印模板已存在！');
	} else if(fCheck(document.forms[0])){
		jQuery("#form1").ajaxSubmit({
			async: false,
			url:"saveTemplatePara",
			dataType:'json',
			type : "post",
			success:function(data){
				if(data.success){
...
function getTemplateList(){
    jQuery.ajax({
		...
		data : {templateType:jQuery("#template_type").val()},
		type : "post",
		dataType : 'json',
		success : function(data) {
			if (data.success) {
				var oOption = null;
				clearAllOptions(document.getElementById("defaultFrom"));
				...
				for(var i in rtnJSONObject){
					...
					document.getElementById("defaultFrom").add(oOption);
				}
			}else {
				alert(data.msg);
			}
