...
根据给定的obj、分隔符，返回组合值
***************************************/
function getValuesByObjs(objs,splitChar){
    var rtnValues="";
    for(var i=0;i<objs.length;i++){
			if(objs[i].checked){
			    rtnValues+=objs[i].value+splitChar;
			}
    }
    return rtnValues==""?rtnValues:rtnValues.substring(0, rtnValues.length-1);
}
/***************************************
两个select之间进行copy
***************************************/
...
判断是否有效的日期格式
***************************************/
function isDateWithFlag(field,splitFlag){
...
function fElementCheck(ele){
	...
	if(tp=="text" || tp=="password" || tp=="textarea" || tp=="select-one" ){
		...
		if(ele.getAttribute("fType")!=null && ele.value.length>0 && ele.getAttribute("fType").length>0){
			...
			if(ele.getAttribute("fType")=="date" && !isDate(ele.value)){
				alert("“"+tip+"”值不是有效的日期格式！");
				ele.focus();
				return false;
			}
		}
